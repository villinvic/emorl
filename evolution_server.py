...
class EvolutionServer:
    ...
    def __init__(self, ID, env_id='Pong-ram-v0', collector_ip=None, psw="", traj_length=10, batch_size=16, max_train=10,
        ...
        if subprocess:
            ...
            if psw != "":
                ssh.tunnel_connection(self.mating_pipe, "tcp://%s:5655" % self.ip, "isys3@%s", password=psw)
                ssh.tunnel_connection(self.evolved_pipe, "tcp://%s:5656" % self.ip, "isys3@%s", password=psw)
            else:
    ...
    def crossover(self, mating):
        ...
        for i in range(0, len(mating)-1, 2):
            ...
            q1 = deepcopy(p1)
            # q2 = deepcopy(p1)
            s = 8019 #25 * 64 + 64*65 + 65 * 6 + 65 * 1  # 5,447 33927 128×128 × 2 +128×2 + 128×6 + 6 + 128 + 1
            c = 0
    ...
    def mutate(self, offspring, intensity=0.005):
        for q in offspring:
            if np.random.random() < self.mutation_rate:
                ...
                gaussian_noised = np.random.normal(loc=q['r'], scale=q['r']*0.4, size=q['r'].shape)
